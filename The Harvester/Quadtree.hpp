...
inline bool AABB(const sf::FloatRect rect1, const sf::FloatRect rect2) {
    return (rect1.position.x + rect1.size.x > rect2.position.x && rect1.position.x < rect2.position.x + rect2.size.x && rect1.position.y + rect1.size.y>rect2.position.y && rect1.position.y < rect2.position.y + rect2.size.y);
}
#include <vector>
...
private:
    ...
    {
    public:
        Node(sf::FloatRect boundary, ushort capacity) : lines(sf::PrimitiveType::LineStrip)
        {
            NE = NW = SE = SW = nullptr;
            ...
            const sf::Vector2f position(boundary.position.x, boundary.position.y);
            const sf::Vector2f size(boundary.size.x, boundary.size.y);

            const sf::Color color = sf::Color::Yellow;

            sf::Vertex vert[5];
            vert[0] = sf::Vertex{ position, color };
            vert[1] = sf::Vertex{ position + sf::Vector2f(size.x, 0), color };
            vert[2] = sf::Vertex{ position + size, color };
            vert[3] = sf::Vertex{ position + sf::Vector2f(0, size.y), color };
            vert[4] = sf::Vertex{ position, color };

            this->lines.append(vert[0]);
            this->lines.append(vert[1]);
            this->lines.append(vert[2]);
            this->lines.append(vert[3]);
            this->lines.append(vert[4]);
        }
        ...
        void subdivide()
        {
            const float x1 = boundary.position.x;
            const float y1 = boundary.position.y;
            const float x2 = boundary.size.x + x1;
            const float y2 = boundary.size.y + y1;

            // setting sides for each rectangle appropriately
            const sf::Vector2f size(boundary.size.x / 2, boundary.size.y / 2);
...
void QuadTree<DataType>::insert_helper(DataType* object, NodePtr node)
{
    ...
    if (!AABB(node->boundary, object->getGlobalBounds()))
        return;
...
{
    if (!AABB(node->boundary,range))
        return;
    else
    {
        for (ushort i = 0; i < node->objects.size(); i++)
        {
            if (AABB(range,node->objects[i]->getGlobalBounds()))
            {
